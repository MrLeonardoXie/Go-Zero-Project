...
func (l *ArticlesLogic) Articles(in *pb.ArticlesRequest) (*pb.ArticlesResponse, error) {
	...
	if len(articleIds) > 0 {
		...
		slices.SortFunc(articles, cmpFunc) //comFunc:返回 <0, 表示 a 应该排在 b 前

		//将排序好的articles加入到curPage
	...
	} else { // Redis中没有找到缓存，需要去MySQL数据库里面找
		// singleFlight: 把同一 key 的并发请求合并成一次实际执行，其它并发请求会等待并复用结果。不同 key 仍会并行执行。
	...
	if !isCache {
		threading.GoSafe(func() { //异步将从Mysql读取到的200条数据，写入Redis缓存
...
func (l *ArticlesLogic) cacheArticles(ctx context.Context, uid, cursor, ps int64, sortType int32) ([]int64, error) {
	...
	if b {
		err = l.svcCtx.BizRedis.ExpireCtx(ctx, key, articlesExpire) //给缓存做续期,TTL设置为articlesExpire
		if err != nil {
...
func (l *ArticlesLogic) addCacheArticles(ctx context.Context, articles []*model.Article, userId int64, sortType int32) error {
	...
	key := articlesKey(userId, sortType) //eg.key=biz#articles#1#1, 通过这种key在redis寻找缓存
	for _, article := range articles {
		var score int64
	...
	return l.svcCtx.BizRedis.ExpireCtx(ctx, key, articlesExpire) //把 key 的 TTL 设为 articlesExpire
}
