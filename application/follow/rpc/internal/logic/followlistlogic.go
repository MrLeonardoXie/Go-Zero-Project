...
import (
	...
	"leonardo/application/follow/code"
	"leonardo/application/follow/rpc/internal/model"
	"leonardo/application/follow/rpc/internal/svc"
	"leonardo/application/follow/rpc/internal/types"
	"leonardo/application/follow/rpc/pb"
...
func (l *FollowListLogic) FollowList(in *pb.FollowListRequest) (*pb.FollowListResponse, error) {
	...
	if len(followUserIds) > 0 {
		...
		//缓存分页命中但这一页没有数据时，直接返回空结果，避免再查库
		//if len(followUserIds) == 0 {
		//	return &pb.FollowListResponse{}, nil
		//}
		if len(followUserIds) == 0 {
			return &pb.FollowListResponse{IsEnd: true, Items: []*pb.FollowItem{}}, nil
		} //我的改进：标记IsEnd=true,如果不为true，那么客户端会一直请求，一直返回空结果，造成资源浪费
		follows, err = l.svcCtx.FollowModel.FindByFollowedUserIds(l.ctx, in.UserId, followUserIds) //[](UserId, FollowedUserId, ...)
		if err != nil {
			l.Logger.Errorf("[FollowList] FollowModel.FindByFollowedUserIds error: %v req: %v", err, in)
			return nil, err
		}
		for _, follow := range follows {
			followedUserIds = append(followedUserIds, follow.FollowedUserID) //[]int64(followedUserId)
			curPage = append(curPage, &pb.FollowItem{
				Id:             follow.ID,
				FollowedUserId: follow.FollowedUserID,
				CreateTime:     follow.CreateTime.Unix(),
			}) //curPage = []FollowItem(UserId, FollowedUserId, CreateTime...)
		}
	} else {
	...
	fc, err := l.svcCtx.FollowCountModel.FindByUserIds(l.ctx, followedUserIds) //[]*FollowCount(UserId, FollowCount, FansCount)
	if err != nil {
		l.Logger.Errorf("[FollowList] FollowCountModel.FindByUserIds error: %v followedUserIds: %v", err, followedUserIds)
	}
	uidFansCount := make(map[int64]int) //[]map(userid:FansCount)
	for _, f := range fc {
		uidFansCount[f.UserID] = f.FansCount
	}
	for _, cur := range curPage { //FollowItem中赋值FansCount
	...
	ret := &pb.FollowListResponse{
		...
		Cursor: cursor,
		Id:     lastId,
		Items:  curPage, //[]FollowItem(UserId, FollowedUserId, FansCount, CreateTime...)
	}

	if !isCache {
		threading.GoSafe(func() { //异步写入
