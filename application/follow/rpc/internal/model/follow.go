...
/*
	TableName() 是在实现 GORM 约定的接口 schema.Tabler（通常写作 Tabler）：
	type Tabler interface {
       TableName() string
	}
	当你的模型实现了这个方法，GORM 就用你返回的表名；
*/
...
func (m *FollowModel) Insert(ctx context.Context, data *Follow) error {
	return m.db.WithContext(ctx).Create(data).Error //加上WithContext(ctx), 否则traces就丢失了
}
