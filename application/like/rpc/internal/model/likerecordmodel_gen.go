...
var (
	...
	cacheLeonardoLikeLikeRecordIdPrefix               = "cache:leonardoLike:likeRecord:id:"
	cacheLeonardoLikeLikeRecordBizIdObjIdUserIdPrefix = "cache:leonardoLike:likeRecord:bizId:objId:userId:"
)
...
func (m *defaultLikeRecordModel) Delete(ctx context.Context, id int64) error {
	...
	leonardoLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheLeonardoLikeLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	leonardoLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheLeonardoLikeLikeRecordIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, leonardoLikeLikeRecordBizIdObjIdUserIdKey, leonardoLikeLikeRecordIdKey)
	return err
}
...
func (m *defaultLikeRecordModel) FindOne(ctx context.Context, id int64) (*LikeRecord, error) {
	leonardoLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheLeonardoLikeLikeRecordIdPrefix, id)
	var resp LikeRecord
	err := m.QueryRowCtx(ctx, &resp, leonardoLikeLikeRecordIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultLikeRecordModel) FindOneByBizIdObjIdUserId(ctx context.Context, bizId string, objId int64, userId int64) (*LikeRecord, error) {
	leonardoLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheLeonardoLikeLikeRecordBizIdObjIdUserIdPrefix, bizId, objId, userId)
	var resp LikeRecord
	err := m.QueryRowIndexCtx(ctx, &resp, leonardoLikeLikeRecordBizIdObjIdUserIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultLikeRecordModel) Insert(ctx context.Context, data *LikeRecord) (sql.Result, error) {
	leonardoLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheLeonardoLikeLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	leonardoLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheLeonardoLikeLikeRecordIdPrefix, data.Id)
...
func (m *defaultLikeRecordModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLeonardoLikeLikeRecordIdPrefix, primary)
}
