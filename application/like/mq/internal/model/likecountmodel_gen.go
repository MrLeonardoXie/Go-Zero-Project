...
var (
	...
	cacheLeonardoLikeLikeCountIdPrefix         = "cache:leonardoLike:likeCount:id:"
	cacheLeonardoLikeLikeCountBizIdObjIdPrefix = "cache:leonardoLike:likeCount:bizId:objId:"
)
...
func (m *defaultLikeCountModel) Delete(ctx context.Context, id int64) error {
	...
	leonardoLikeLikeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheLeonardoLikeLikeCountBizIdObjIdPrefix, data.BizId, data.ObjId)
	leonardoLikeLikeCountIdKey := fmt.Sprintf("%s%v", cacheLeonardoLikeLikeCountIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, leonardoLikeLikeCountBizIdObjIdKey, leonardoLikeLikeCountIdKey)
	return err
}
...
func (m *defaultLikeCountModel) FindOne(ctx context.Context, id int64) (*LikeCount, error) {
	leonardoLikeLikeCountIdKey := fmt.Sprintf("%s%v", cacheLeonardoLikeLikeCountIdPrefix, id)
	var resp LikeCount
	err := m.QueryRowCtx(ctx, &resp, leonardoLikeLikeCountIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultLikeCountModel) FindOneByBizIdObjId(ctx context.Context, bizId string, objId int64) (*LikeCount, error) {
	leonardoLikeLikeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheLeonardoLikeLikeCountBizIdObjIdPrefix, bizId, objId)
	var resp LikeCount
	err := m.QueryRowIndexCtx(ctx, &resp, leonardoLikeLikeCountBizIdObjIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultLikeCountModel) Insert(ctx context.Context, data *LikeCount) (sql.Result, error) {
	leonardoLikeLikeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheLeonardoLikeLikeCountBizIdObjIdPrefix, data.BizId, data.ObjId)
	leonardoLikeLikeCountIdKey := fmt.Sprintf("%s%v", cacheLeonardoLikeLikeCountIdPrefix, data.Id)
...
func (m *defaultLikeCountModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLeonardoLikeLikeCountIdPrefix, primary)
}
